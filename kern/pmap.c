...
attach_region(uintptr_t start, uintptr_t end, enum PageState type) {
    ...
    while (iter < end) {
        uintptr_t remaining = end - iter;
        int max_class = 0;
        
        for (int class = 0; class <= MAX_ALLOCATION_CLASS; class++) {
            uintptr_t class_size = CLASS_SIZE(class);
            uintptr_t class_mask = CLASS_MASK(class);
            
            if ((iter & class_mask) == 0 && remaining >= class_size) {
                max_class = class;
            } else {
                break;
            }
        ...
        if (!page_lookup(NULL, iter, max_class, type, 1)) {
            bool attached = false;
            for (int class = max_class; class >= 0; class--) {
                uintptr_t class_size = CLASS_SIZE(class);
                if (remaining >= class_size) {
                    if (page_lookup(NULL, iter, class, type, 1)) {
                        iter += class_size;
                        attached = true;
                        break;
                    }
...
dump_virtual_tree(struct Page *node, int class) {
    ...
    if (node->state == MAPPING_NODE) {
        uintptr_t vstart = (uintptr_t)(node->addr << CLASS_BASE);
        uintptr_t vend   = vstart + CLASS_SIZE(node->class);
        uintptr_t pstart = (uintptr_t)(node->phy->addr << CLASS_BASE);
        uintptr_t pend   = pstart + CLASS_SIZE(node->phy->class);

        cprintf("- [V: %016lx - %016lx] -> [P: %016lx - %016lx]  | class=%2d | refc=%d\n",
                vstart, vend - 1, pstart, pend - 1, node->class, node->refc);
    } else if (node->state == INTERMEDIATE_NODE) {
        uintptr_t vstart = (uintptr_t)(node->addr << CLASS_BASE);
        uintptr_t vend   = vstart + CLASS_SIZE(node->class);

        cprintf("* Intermediate node [%016lx - %016lx] | class=%2d\n",
                vstart, vend - 1, node->class);
    } else {
        cprintf("* Unknown node type (state=%d) | class=%2d\n",
                node->state, node->class);
    }
...
map_page(struct AddressSpace *spc, uintptr_t addr, struct Page *page, int flags) {
    ...
    if (!(pde & PTE_P)) {
        ...
        pde_t old_pde = pd[pdi0];
        if (alloc_pt(pd + pdi0) < 0)
            return -E_NO_MEM;

        pte_t *pt = KADDR(PTE_ADDR(pd[pdi0]));
        if (alloc_fill_pt(pt, old_pde & ~PTE_PS, 4 * KB, 0, PT_ENTRY_COUNT) < 0)
            return -E_NO_MEM;
    }
...
init_memory(void) {
    ...
    res = map_physical_region(&kspace, KERN_BASE_ADDR, (uintptr_t)0,
            max_memory_map_addr, PROT_R | PROT_W | ALLOC_WEAK);
    assert(!res);
    ...
    res = map_physical_region(&kspace, (uintptr_t)__text_start,
            PADDR(__text_start), (__text_end - __text_start), PROT_RWX);
    assert(!res);
    ...
    res = map_physical_region(&kspace, KERN_STACK_TOP - KERN_STACK_SIZE,
            PADDR(bootstack), KERN_STACK_SIZE, PROT_R | PROT_W);
    assert(!res);
    res = map_physical_region(&kspace, KERN_PF_STACK_TOP - KERN_PF_STACK_SIZE,
            PADDR(pfstack), KERN_PF_STACK_SIZE, PROT_R | PROT_W);
    assert(!res);
    ...
    for (; mstart < mend; mstart = (void *)((uint8_t *)mstart + uefi_lp->MemoryMapDescriptorSize)) {
        if (mstart->Attribute & EFI_MEMORY_RUNTIME) {
            ...
            res = map_physical_region(&kspace, mstart->VirtualStart, mstart->PhysicalStart, 
    ...
    res = map_physical_region(&kspace, X86ADDR(KERN_BASE_ADDR), (uintptr_t)0, 
